/* autogenerated by Processing revision 1293 on 2025-07-21 */
...
public void detectBeat(float level) {
  int now = millis();
  if (level > 0.6f && now - lastBeatTime > beatCooldown) {
    inf = 5;
    int temp = bgColor;
    bgColor = textColor;
    textColor = temp;
...
class ShellLine {
  ...
  public void update() {
    if (lineTimeout > 0) {
      float age = (millis() - timeAdded) / 1000.0f;
...
public void addShellLine(String line) {
  shellLines.add(new ShellLine(line));
  addToOutput(line);
}

public void drawShellBox() {
...
public void shellAppend(String line) {
  if (!line.trim().equals("")) {
    addShellLine(line);
  }
}
...
public void startTitle(String s) {
  fullTitle = s.replace("\\n", "\n");
...
public void drawTypedCharacters() {
  fill(textColor, 100);
...
public void drawFallingCharacters() {
  ...
  for (int i = 0; i < w; i++) {
    if (input.size() <= i) {
      float energy = 0;
      if (fft != null) {
        int bandIndex = PApplet.parseInt(map(i, 0, w, 0, bands - 1));
      ...
      for (int j = 0; j < bars.size(); j++) {
        text(barChars.get(j), i * size, (bars.get(j) + PApplet.parseInt(raindrops[i] / 20)) * size);
      }

      text(range[PApplet.parseInt(random(0, range.length))], i * size, PApplet.parseInt(raindrops[i]) * size);
      text(range[PApplet.parseInt(random(0, range.length))], i * size, PApplet.parseInt(r2[i]) * size);
    }
  }
}

public void drawRainbowCharacters() {
  colorMode(HSB);
  for (int j = 0; j < bars.size(); j++) {
    bars.set(j, bars.get(j) + speed);
    if (bars.get(j) > h + 2) {
...
public void drawCommandOutputs() {
  textSize(size);
  for (int i = 0; i < outputProg.size(); i++) {
    if (outputProg.get(i) < h + 3) {
      ...
      int yPos = PApplet.parseInt(outputProg.get(i)) * size;
      fill(bgColor, 200);
      rect(xPos, yPos - size, size * outputs.get(i).length(), size);
      fill(textColor);
      text(outputs.get(i), xPos, PApplet.parseInt(yPos));
    }
...
public void processCommand(ArrayList<Character> custom, boolean clear) {
  String inp = "";
  ...
  if (!inp.isEmpty()) {
    ...
    case "size":
      try {
        int newSize = PApplet.parseInt(parameter.trim());
        if (newSize > 0) {
          initialize(newSize);
    ...
    case "fps":
      try {
        float fps = PApplet.parseFloat(parameter.trim());
        if (fps > 0) {
          frameRate(fps);
    ...
    case "speed":
      try {
        float spd = PApplet.parseFloat(parameter.trim());
        speed = spd;
    ...
    case "repeat":
      ...
      if (parts.length > 0 && !parts[0].isEmpty()) {
        try {
          times = max(1, PApplet.parseInt(parts[0]));
        }
        catch (NumberFormatException e) {
      ...
      if (parts.length > 1) {
        try {
          delay = max(0, PApplet.parseInt(parts[1]));
        }
        catch (NumberFormatException e) {
    ...
    case "timeout":
      try {
        lineTimeout = PApplet.parseFloat(parameter);
      }
      catch(NumberFormatException e) {
      }
      break;
...
public void loadState() {
  ...
  lastCommand = save[0];
  int s = PApplet.parseInt(save[1]);
  if (s > 0) initialize(s);

  t_framerate = PApplet.parseFloat(save[2]);
  frameRate(t_framerate);

  speed = PApplet.parseFloat(save[3]);
  range = save[4].toCharArray();
  textColor = PApplet.parseInt(save[5]);
  bgColor = PApplet.parseInt(save[6]);
  lineTimeout = PApplet.parseFloat(save[7]);
  fullTitle = save[8];
  if (!fullTitle.equals("")) {
    startTitle(fullTitle);
  }
...
public String setColor(String parameter) {
  String[] rgb;
...
public String setBackgroundColor(String parameter) {
...
public String setFade(String parameter) {
  try {
...
public boolean isAlphanumericOrSymbol(char key) {
  if (
    (key >= 'A' && key <= 'Z') ||
    (key >= 'a' && key <= 'z') ||
    (key >= '0' && key <= '9') ||
...
class Drop {
  ...
  public void display() {
    fill(textColor);
    text(this.ch, PApplet.parseInt(this.x) * size, PApplet.parseInt(this.y) * size);

    if (this.y >= h) {
      if (pows[PApplet.parseInt(this.x)] <= 0) {
      } else {
        explosions.add(
          new Explosion(
          this.x * size,
        ...
        for (var i = 0; i < w; i++) {
          pows[i] -= (255 / abs(this.x - i + 0.001f)) * 2;
        }
        this.destroy();
      }
      if (this.y > h + 5) {
...
class Explosion {
  ...
  Explosion(float _x, float  _y, float  _radius, float _particles, int _color) {
    this.x = _x;
    this.y = _y;
    this.radius = _radius;
    this.t = 1;
  ...
  public void display() {
    if (this.fillcolor == textColor) {
      score += PApplet.parseInt(this.radius);
    }
    fill(this.fillcolor);
    for (int i = 0; i < this.ptcles.size(); i++) {
      textSize(this.radius * random(0.8f, 1.2f));
      float xpos = PApplet.parseInt((this.ptcles.get(i).x - this.radius / 2) / size) * size;
      float ypos = PApplet.parseInt((this.ptcles.get(i).y - this.radius / 2) / size) * size;
      text(
        str(range[PApplet.parseInt(constrain(PApplet.parseInt(xpos + ypos) % range.length, 0, range.length))]),
        xpos,
        ypos
        );
    }
...
noSmooth(); }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#000000", "--hide-stop", "Matrix" };
    if (passedArgs != null) {
